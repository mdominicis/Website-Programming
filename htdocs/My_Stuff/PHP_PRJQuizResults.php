...
echo "<table><tr class=heading><td> Question <td> Answer <td> YOUR Answer <td> Result</tr>";
...
for($r=0;$r<$rowCount;$r++)
{
	...
	echo "<tr>";
	    for($c=1;$c<$colCount;$c++) //loops through each column
	    {
	         echo "<td>".$row[$c];
                
	    }
	 
			if(strtoupper($row[2]) == strtoupper($ansArray[$row[0]])){
                         
			   echo "<td>".$ansArray[$row[0]];
			   echo "<td class=\"green\">correct";
			   $cntCorrect++;
			}
			else { 
			 
			   echo "<td>".$ansArray[$row[0]];
			   echo "<td class=\"red\">Incorrect";
			   $cntInCorrect++;
			}
...
echo "</table>";
	...
	echo "<br>Correct: ".$cntCorrect."<br>";
	echo "Incorrect: ".$cntInCorrect."<br>";
	echo "Your score: (" .$cntCorrect. "/" .$rowCount. ") " .(($cntCorrect/$rowCount)*100) ."% (C) <br>";
?>
